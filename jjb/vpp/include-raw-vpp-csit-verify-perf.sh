...
if [[ -v CSIT_REF ]]; then
    # FIXME: Why is RSA key check failing of fd.io sandbox?
    GIT_SSH_COMMAND="ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no" git fetch ssh://rotterdam-jobbuilder@gerrit.fd.io:29418/csit $CSIT_REF && git checkout FETCH_HEAD
else
