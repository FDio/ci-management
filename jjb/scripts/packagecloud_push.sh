...
create_deb_push_cmds()
{
    ...
    DEBS=$(find . -type f -iname '*.deb' | grep -v vpp-ext-deps || true)
    push_cmd="package_cloud push ${PCIO_CO}/${STREAM}/$1/${FACTER_LSBNAME}/main/ ${DEBS}"
    ...
    if [ -n "$ext_deps_pkg}" ] ; then
        push_ext_deps_cmd="package_cloud push ${PCIO_CO}/${STREAM}/$1/${FACTER_LSBNAME}/main/ ${ext_deps_pkg}"
    fi
}
...
create_rpm_push_cmds()
{
    ...
    RPMS=$(find . -type f -iregex '.*/.*\.\(s\)?rpm' | grep -v vpp-ext-deps || true)
