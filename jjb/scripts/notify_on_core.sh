...
send_notify() {
    ...
    echo "WEBEX_TEAMS_ROOM_ID=${WEBEX_TEAMS_ROOM_ID}"
#    curl "https://api.ciscospark.com/v1/messages" -X "POST" -H "Authorization: Bearer ${SECRET_WEBEX_TEAMS_ACCESS_TOKEN}" -H "Content-Type: application/json" --data '{"roomId":"Y2lzY29zcGFyazovL3VzL1JPT00vNmQ3NzllOTAtYjk0NS0xMWViLTgxYzAtMzdhNmIxZThmMzNi", "markdown": "'"${WEBEX_TEAMS_MESSAGE}"'"}' || true
    curl "https://api.ciscospark.com/v1/messages" -X "POST" -H "Authorization: Bearer ${SECRET_WEBEX_TEAMS_ACCESS_TOKEN}" -H "Content-Type: application/json" --data '{"roomId":"Y2lzY29zcGFyazovL3VzL1JPT00vNmQ3NzllOTAtYjk0NS0xMWViLTgxYzAtMzdhNmIxZThmMzNi", "markdown": "Not a real message yet."}' || true
}
