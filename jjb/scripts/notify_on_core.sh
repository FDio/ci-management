...
#    curl https://api.ciscospark.com/v1/messages -X POST -H "Authorization: Bearer ${SECRET_WEBEX_TEAMS_ACCESS_TOKEN}" -H "Content-Type: application/json" --data '{"roomId":"'"${WEBEX_TEAMS_ROOM_ID}"'", "markdown": "'"${WEBEX_TEAMS_MESSAGE}"'"}' || true
    curl https://api.ciscospark.com/v1/messages -X POST -H "Authorization: Bearer ${SECRET_WEBEX_TEAMS_ACCESS_TOKEN}" -H "Content-Type: application/json" --data '{"roomId":"'"${WEBEX_TEAMS_ROOM_ID}"'", "markdown": "'"Not a real message yet."'"}' || true
}
