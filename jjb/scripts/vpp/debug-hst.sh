...
make_build() {
    ...
    if ! make build; then
        BUILD_ERROR="FAILED 'make build' (debug)"
        return
    fi
    if ! make -C "$HST_DIR" install-deps build-debug ; then
        BUILD_ERROR="FAILED 'make -C $HST_DIR install-deps build-debug'"
        return
    fi
}

hst_test_debug() {
    if ! make VERBOSE=true VPPSRC="$(pwd)" -C "$HST_DIR" test ; then
