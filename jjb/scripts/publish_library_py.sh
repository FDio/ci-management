...
def upload_recursive(s3_resource, s3_bucket, src_fpath, s3_path):
    ...
    :type s3_path: str
    """
    for leaf_dir, _, filenames in os.walk(src_fpath):
        for filename in filenames:
            upload(
                s3_resource=s3_resource,
                s3_bucket=s3_bucket,
                src_fpath=os.path.join(leaf_dir, filename),
                s3_path=urljoin(s3_path, os.path.normpath(
                    os.path.join(leaf_dir.replace(src_fpath, u""), filename)
                ))
            )
