...
export EXECUTOR_CLASSES="builder csit csit_shim csit_dut"
export EXECUTOR_OS_ARCHS="x86_64 aarch64"
case "$OS_ARCH" in
    x86_64)
...
executor_list_os_names() {
    ...
    # Verify dependency on lib_vpp.sh: assumption is VPP defines supported OSes
    # TODO: refactor to include restrictions per executor class.
    if [ -n "$(alias lib_vpp_imported 2> /dev/null)" ] ; then
        echo
        echo "Valid executor OS names:"
...
executor_bad_os_name() {
    echo "ERROR: Invalid executor OS name: $1!"
    executor_list_os_names
    echo
}
