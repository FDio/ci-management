...
csit_pip_cache() {
    ...
    if [ -f "$csit_dir/VPP_REPO_URL" ] \
        ...
        local PIP="python -m pip --disable-pip-version-check"
        ...
        if [ "$OS_ARCH" = "aarch64" ] ; then
            ...
            [ -n "$numpy_ver" ] && $PIP install $numpy_ver 2>&1 | tee -a $bld_log
        fi
        $PIP install -r $PYTHONPATH/requirements.txt 2>&1 | tee -a $bld_log
        $PIP install -r $PYTHONPATH/tox-requirements.txt 2>&1 | tee -a $bld_log
        $PIP install -r $PRESENTATION_DIR/requirements.txt 2>&1 | tee -a $bld_log
