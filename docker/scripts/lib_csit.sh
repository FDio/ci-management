...
csit_pip_cache() {
    ...
    if [ -f "$CSIT_DIR/VPP_REPO_URL" ] \
        ...
        # Use csit activate_virtualenv() to create env and
        # install $CSIT_DIR/requirements.txt
        source "$common_sh" && activate_virtualenv
        ...
        report_dir="$(grep 'DIR\[WORKING\]=' $report | mawk -F'=' '{print $2}')"

        # Clean up virtualenv directories
        deactivate
        rm -rf "$report_dir" "$PYTHONPATH/env"
