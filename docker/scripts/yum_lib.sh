# yum_lib.sh - Docker build script yum library.
#              For import only.

# Don't import more than once.
if [ -n "$(alias yum_lib_imported 2> /dev/null)" ] ; then
  return 0
fi
alias yum_lib_imported=true
...
dump_yum_package_list() {
  ...
  yum list installed > "$BUILD_LOG_DIR/$FDIOTOOLS_IMAGENAME-$branchname-yum-packages.log"
}
...
#
# Note: This MUST come before populating the
#       OS packaes for VPP, otherwise the
#       the docker image build fails on
#       aarch64.
