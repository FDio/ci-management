...
make_vpp_hst() {
    ...
    if grep -q 'test' <<< "$target" ; then
        hst_test="TEST=EchoBuiltinTest"
        description="$description ${hst_test}"
    fi
    description="$description' in $(pwd) ($branch)"
    echo_log -e "    Starting  $description..."
    make VERBOSE=true VPPSRC="$DOCKER_VPP_DIR" -C "$HST_DIR" $target $hs_test 2>&1 | tee -a "$bld_log"
    git checkout -q -- .
    echo_log "    Completed $description!"
}
