...
inspect_images() {
    echo "$1Production Docker Images:"
    echo "$short_bar"
    if [ -n "$image_tags_new" ] ; then
        print_image_list "NEW" "$docker_id_new" "$image_tags_new"
        echo
    fi
    print_image_list "prod-$image_arch" "$docker_id_prod" "$image_tags_prod"
    echo
    print_image_list "prod-prev-$image_arch" "$docker_id_prev" \
                     "$image_tags_prev"
    echo -e "$short_bar\n"
}
