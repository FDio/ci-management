...
{
    lf_venv=${LF_VENV:-"/root/lf-venv"}
    if [ ! -d "$lf_venv/bin" ] ; then
        echo "ERROR: Missing lf venv ($lf_venv/bin), LF_VENV = '$LF_VENV'!"
        return 1
    fi
    ...
    # Only install/upgrade packages in docker image builder
    if grep -qe 'dbld_lfit_requirements\.sh' <<<"${BASH_SOURCE[@]}" ; then
        ...
        case $python in
            python3*)
                ...
                "$lf_venv/bin/pip" install --upgrade pip virtualenv || return 1
                if [[ -z $pkg_list ]]; then
                ...
                "$lf_venv/bin/pip" install --upgrade --upgrade-strategy \
                                   eager $pkg_list || return 1
                ;;
            *)
    ...
    # Activate the virtual env without executing 'source $lf_venv/bin/activate'
    # This means that deactivate() will not get created and
    # PATH must be modified manually to deactivate the virtual env.
    if $set_path; then
