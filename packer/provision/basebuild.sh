...
rh_systems() {
    ...
    python-pip openssl-devel mysql-devel"
    yum install -y ${RH_PYTHON_PKGS}

    # RH Install Documentation packages
    echo "---> Installing RH documentation packages $(date +'%Y%m%dT%H%M%S')"
    RH_DOC_PKGS="doxygen graphviz python-pyparsing python-jinja2 \
    asciidoc dblatex source-highlight python-sphinx" 
...
ubuntu_systems() {
    ...
    PYTHON_PKGS="python-dev python-virtualenv python-setuptools \
    python-pip libssl-dev libmysqlclient-dev"
    ...
    TOOL_PKGS="iproute2 ethtool vlan bridge-utils vim gdb \
    lsb-release"
