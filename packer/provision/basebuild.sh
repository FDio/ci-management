...
rh_systems() {
    ...
    # Install build tools
    TOOLS_PKGS="@development redhat-lsb glibc-static \
    java-1.8.0-openjdk-devel yum-utils openssl-devel \
    apr-devel indent"
    yum install -y ${TOOLS_PKGS}

    # Install python dependencies
    PYTHON_PKGS="python-devel python-virtualenv python-setuptools \
    python-pip"
    yum install -y ${PYTHON_PKGS}

    # Build dependencies for Python packages
    PYTHON_DEP="openssl-devel mysql-devel"
    yum install -y ${PYTHON_DEP}

    # Install Documentation packages
    DOC_PKGS="doxygen graphviz python-pyparsing python-jinja2"
    yum install -y install ${DOC_PKGS}

    # Install GCC packages
    GCC_PKGS="gcc c++ cmake"
    yum install -y ${GCC_PKGS}

    # Install components to build Ganglia modules
    GANGLIA_MODS="libconfuse-devel ganglia-devel mock"
    yum install -y --enablerepo=epel ${GANGLIA_MODS}

    # Install module for VPP project
    echo uio_pci_generic >> /etc/modules

    #Install VPP packages to shorten build times
    echo "---> Installing VPP DEB_DEPENDS packages"
    VPP_PKGS="curl build-essential autoconf automake bison libssl-dev \
    ...
    DEBUG_PKGS="glibc-2.17-106.el7_2.4.x86_64 \
    openssl-libs-1.0.1e-51.el7_2.4.x86_64 zlib-1.2.7-15.el7.x86_64"
    debuginfo-install -y ${DEBUG_PKGS}
...
ubuntu_systems() {
    ...
    # Build dependencies for Python packages
    PYTHON_DEP="libssl-dev libmysqlclient-dev"
    apt install -y ${PYTHON_DEP}
    ...
    DEB_DPDK_PKGS="dpkg-dev devscripts pristine-tar dh-python inkscape \
    libcap-dev libxen-dev libxenstore3.0 python-sphinx"
